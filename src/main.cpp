...
#if defined(NDEBUG)
# error "BrovaIdeaCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfa3b6e479fbed7c5ffd11633d6e79b5e83f27fa9b11e37e23c6b153eaa7d97c2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BrovaIdeaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BrovaIdeaCoin Signed Message:\n";
...
{
    // BrovaIdeaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 50 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 266667 * COIN;
            }

    // Subsidy is cut in half every 24000 blocks
    nSubsidy >>= (nHeight / 24000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BrovaIdeaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BrovaIdeaCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1515699932, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Blogger raser mot kamelridning";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0488e7bd189a42e005a747d1507b7db6d79f817237d5d8d931eb0c2201f1a1f166e0f76eede9c1489c25a69e691649bd164ab5b7179763f20c2d0d926482c80d03") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515699932;
        ...
        assert(block.hashMerkleRoot == uint256("0x24c640622452e31cb24ed1dadccfc27c82b2849545a326526f51a20ddaf1545e"));
...
unsigned char pchMessageStart[4] = { 0x54, 0xe2, 0x92, 0x68 }; // BrovaIdeaCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BrovaIdeaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BrovaIdeaCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BrovaIdeaCoinMiner(CWallet *pwallet)
{
    printf("BrovaIdeaCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("brovaideacoin-miner");
    ...
    try { loop {
        ...
        printf("Running BrovaIdeaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BrovaIdeaCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BrovaIdeaCoinMiner, pwallet));
}
